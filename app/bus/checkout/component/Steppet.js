  ...
  return (
    <div className="w-full max-w-2xl mx-auto my-10 p-4 bg-gray-100">
      ...
      <div className="mt-4 flex justify-between">
        <button onClick={prevStep} className={`py-2 px-4 rounded-md ${currentStep === 1 ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-500 text-white'}`} disabled={currentStep === 1}>
          Previous
        </button>
        <button onClick={nextStep} className={`py-2 px-4 rounded-md ${currentStep === steps.length ? 'bg-gray-200 text-gray-400 cursor-not-allowed' : 'bg-blue-500 text-white'}`} disabled={currentStep === steps.length}>
          Next
        </button>
      </div>
    </div>
